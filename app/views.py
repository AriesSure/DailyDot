...
def todo_list():
    try:
        # Get all todos for the user, ordered by creation time (newest first)
...
def statistics():
    try:
        ...
        habit_stats = []
        for habit in habits:
            habit_records = [r for r in recent_records if r.habit_id == habit.id]
            habit_stats.append({
                'name': habit.habit_name,
                'total_checkins': len(habit_records),
                'icon': habit.icon
            })
